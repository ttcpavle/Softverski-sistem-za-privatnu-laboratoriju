...
public class DodajStavkuZahtevaZaAnalizu extends OpstaSO {
    ...
    protected Exception preduslovi(OpstiDomenskiObjekat odo, DBBroker dbb) {
        ...
        if (s.getZahtev() == null || s.getZahtev().getIdZahtev() <= 0) {
            return new Exception("Stavka mora biti povezana sa zahtevom");
        }
    ...
    protected Response izvrsenjeSO(OpstiDomenskiObjekat odo, DBBroker dbb) {
        ...
        StavkaZahteva s = (StavkaZahteva) odo;
        boolean result = dbb.pamtiSlog(s);
        if (!result) {
            return new Response(null, new Exception("Greska pri dodavanju stavke zahteva (rbStavka=" + s.getRbStavka() + ")"), false);
        }
