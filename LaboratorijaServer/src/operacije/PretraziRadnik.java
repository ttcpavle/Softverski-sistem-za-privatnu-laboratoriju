/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package operacije;

import communication.Response;
import database.DBBroker;
import domen.OpstiDomenskiObjekat;
import domen.Radnik;

/**
 *
 * @author totic
 */
public class PretraziRadnik extends OpstaSO{

    @Override
    protected Exception preduslovi(OpstiDomenskiObjekat odo, DBBroker dbb) {
        if(!(odo instanceof Radnik)){
            return new Exception("Prosledjeni objekat nije radnik");
        }
        return null;
    }

    @Override
    protected Response izvrsenjeSO(OpstiDomenskiObjekat odo, DBBroker dbb) {
        Exception e = preduslovi(odo, dbb);
        if(e != null){
            return new Response(null, e, false);
        }
        
        boolean result = dbb.nadjiSlog(odo);
        if(result){
            Radnik r = (Radnik) dbb.getRezultat();
            return new Response(r, null, result);
        }
        return new Response(null,new Exception("Nije nadjen radnik"),true);
    }
    
}
